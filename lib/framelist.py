...
class FrameList(list):
    ...
    def select(self, n):
        if not 0 <= n <= len(self)-1:
            ...
            print "IndexError: Trying to select inexistent frame."
            return
        self.idx = n
    ...
    def goto_next(self, with_cel=False):
        if with_cel:
            ...
            if next_frame is None:
                ...
                print "IndexError: There is no next frame with cel."
            else:
                self.idx = self.index(next_frame)
            return
        if not self.has_next():
            #raise IndexError("Trying to go to next at the last frame.")
            print "IndexError: Trying to go to next at the last frame."
        else:
            self.idx += 1
    ...
    def goto_previous(self, with_cel=False):
        if with_cel:
            ...
            if prev_frame is None:
                ...
                print "IndexError: There is no previous frame with cel."
            else:
                self.idx = self.index(prev_frame)
            return
        if not self.has_previous():
            #raise IndexError("Trying to go to previous at the first frame.")
            print "IndexError:Trying to go to previous at the first frame."
        else:
            self.idx -= 1
    ...
    def goto_next_key(self):
        ...
        if f is None:
            ...
            print "IndexError: Trying to go to inexistent next keyframe."
        else:
    ...
    def goto_previous_key(self):
        ...
        if f is None:
            print "Trying to go to inexistent previous keyframe."
        else:
            self.idx = self.index(f)
